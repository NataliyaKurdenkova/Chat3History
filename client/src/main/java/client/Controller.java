...
public class Controller implements Initializable {
    ...
    private static PrintWriter outHistory;
                    ...
                    while (true) {
                        ...
                        if (str.startsWith("/")) {
                            ...
                            if (str.startsWith(Command.AUTH_OK)) {
                                ...
                                //выводим 100 строчек
                                textArea.appendText(getHistory(login));
                                try {
                                    outHistory = new PrintWriter(new FileOutputStream(createFile(login), true), true);
    ...
    public static void printMess(String msg) {
        outHistory.println(msg);
    }

    private static String createFile(String login) {
        String nameFile = "client/src/main/java/client/history/history_";
        return nameFile + login + ".txt";

    }

    public static String getHistory(String login) {
        if (!Files.exists(get(createFile(login)))) {
            return "";
        }

        String h="";
        StringBuilder sb = new StringBuilder();
        //List<String> historyAll=new ArrayList<>();

        try {
            BufferedReader reader = new BufferedReader(new FileReader(String.valueOf(get(createFile(login)))));


            while(true){
                try {
                    if (!((h=reader.readLine())!=null)) break;
                } catch (IOException e) {
                    e.printStackTrace();
                }
